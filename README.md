...
2. **00991A 加碼**：主動復華未來 50 較前一日加碼（含新進場）的個股，且週線由下轉彎向上（復華 API 全自動，免設定）
3. **00993A 加碼**：主動安聯台灣（安聯台股團隊操盤）加碼個股，且週線由下轉彎向上——需設定一次 `ALLIANZ_PCF_URL`：
   用 Chrome 開 https://etf.allianzgi.com.tw/list-trade 選 00993A，F12 → Network → 找到回傳持股 JSON 的 API 網址，
   填入 repo Settings → Secrets and variables → Actions → Variables，名稱 `ALLIANZ_PCF_URL`。
   若網址含日期可改寫成 `{date}` 佔位符（例如 `...&date={date}`），腳本每天自動代入當天日期。

> 註：安聯台灣科技基金為傳統共同基金，僅每月公布前十大持股，無法每日追蹤，故以同團隊的主動式 ETF 00993A 替代。
